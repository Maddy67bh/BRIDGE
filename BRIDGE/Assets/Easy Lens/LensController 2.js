...
script.createEvent("OnStartEvent").bind(function() {
    ...
    // Bind buttons with guards to avoid undefined .onTap errors
    if (script.msg_btn_1 && script.msg_btn_1.onTap) {
        script.msg_btn_1.onTap.add(function() {
            loadDemo(0);
        });
    } else { debugWarnOnce("msg_btn_1.onTap unavailable; skipping bind"); }

    if (script.msg_btn_2 && script.msg_btn_2.onTap) {
        script.msg_btn_2.onTap.add(function() {
            loadDemo(1);
        });
    } else { debugWarnOnce("msg_btn_2.onTap unavailable; skipping bind"); }

    if (script.msg_btn_3 && script.msg_btn_3.onTap) {
        script.msg_btn_3.onTap.add(function() {
            loadDemo(2);
        });
    } else { debugWarnOnce("msg_btn_3.onTap unavailable; skipping bind"); }

    if (script.reset_btn && script.reset_btn.onTap) {
        script.reset_btn.onTap.add(function() {
            resetWorkflow();
        });
    } else { debugWarnOnce("reset_btn.onTap unavailable; skipping bind"); }

    // Set up touch events for node taps
