...
function pulseOnce() {
    pulseObj.s = 1; if (pulseTween && pulseTween.isPlaying()) { pulseTween.stop(); }
...
function playEnter() {
    enterObj.a = 0; enterObj.s = 0.92; if (enterTween && enterTween.isPlaying()) { enterTween.stop(); }
    enterTween = new TWEEN.Tween(enterObj)
        .to({ a: 1, s: 1 }, ENTER_DURATION_MS)
        .easing(TWEEN.Easing.Quadratic.Out)
        .onUpdate(function(){ draw(); })
        .start();
}
...
function handleTap(px, py) {
    for (let i = 0; i < hits.length; i++) {
        const h = hits[i]; if (inRect(px, py, h.rect)) { routeTap(h.id); return; }
    }
}
