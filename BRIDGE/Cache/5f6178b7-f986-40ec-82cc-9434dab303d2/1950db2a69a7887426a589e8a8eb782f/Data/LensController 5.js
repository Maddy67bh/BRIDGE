...
// LinkSpace — Spatial Communication Hub using Canvas + Touch Events (no direct .onTap on SceneObjects)
...
const SHOW_INSTRUCTIONS_TEXT = false; // We will use LinkSpace text blocks instead
...
script.createEvent("OnStartEvent").bind(function() {
    ...
    // Disable legacy BRIDGE UI text to avoid overlap; enable LinkSpace text
    ...
    if (script.linkspace_title) {
        script.linkspace_title.text = "LINKSPACE";
        script.linkspace_title.forceSafeRegion(true);
    }
    if (script.linkspace_subtitle) {
        script.linkspace_subtitle.text = "Connect in Space";
        script.linkspace_subtitle.forceSafeRegion(true);
    }
    if (script.linkspace_hint) {
        script.linkspace_hint.text = "Tap a contact to connect";
        script.linkspace_hint.forceSafeRegion(true);
    }

    // Remove legacy UI Button bindings — all taps go through touch_events hit-testing now
    ...
    // Initial state: show LinkSpace HOME
    resetWorkflow();
    drawUI();
});
