...
//@input Component.ScriptComponent message_input_text
...
// Presets
const PRESETS = ["MEET HERE", "REVIEW THIS", "IMPORTANT", "FOLLOW UP"]; // limited per requirements (DONE handled via COMPLETE)
...
script.createEvent("OnStartEvent").bind(function() {
    ...
    // Ensure editable input text is hidden on start (HOME)
    if (script.message_input_text) { script.message_input_text.enabled = false; }
...
function setState(s) {
    state = s;
    // UI visibility fix: scope editable input only to NEW MESSAGE screen
    if (script.message_input_text) {
        if (state === STATE_NEW) {
            // Enable for NEW MESSAGE; if user not typing yet, show instruction
            script.message_input_text.enabled = true;
            if (!script.message_input_text.text || script.message_input_text.text.trim() === "") {
                script.message_input_text.text = "TYPE A SHORT MESSAGE";
            }
        } else {
            // Hide on HOME / BOARD / DEMO / others
            script.message_input_text.enabled = false;
        }
    }
    draw();
}
