...
// Version 1.1.0
//
// Provides touch eventTypes as a block for AI Lens Creator
//
...
function init() {
    ...
    for (const i in eventTypes) {
        registerEvent(eventTypes[i][0], eventTypes[i][1], eventTypes[i][2]);
    }
    if(global.EasyLens){
        global.EasyLens.VisualEdit.visualEditStateEnabled.addListener((enabled) => {
            veActive = enabled;
        }) 
    }
    setupTouchBlocking();
...
function registerEvent(name, builtinName,hintName) {
    ...
    script.createEvent(builtinName).bind(function(data) {
        ...
        else if(data && data.getTapPosition){
            const pos = data.getTapPosition();
            event.trigger(pos.x,pos.y);           
        }
        else {
            event.trigger();
        }
